...
struct PixelOutput
{
	...
	// But also emits gbuffers :p Maybe switch to deferred shading later...
	GBUFFER0_DATATYPE gbuffer0    : SV_TARGET1;
	GBUFFER1_DATATYPE gbuffer1    : SV_TARGET2;
	
	// Velocity values are in [0, 1) as if render resolution is unscaled.
	// For temporal reprojection, get unscaled screen uv -> apply velocity -> apply prev render scale.
