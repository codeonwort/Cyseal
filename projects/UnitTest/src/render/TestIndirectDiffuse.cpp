...
struct ActualImage_IndirectDiffuse
{
...
class IndirectDiffuseApplication : public ConsoleApplication
{
public:
	IndirectDiffuseApplication(CysealEngine* inEngine, ERenderDeviceRawAPI inGraphicsAPI, ActualImage_IndirectDiffuse* inActualImage, bool bInDiffuseOnly, bool bInUseSTBN)
		: cysealEngine(inEngine)
		, graphicsAPI(inGraphicsAPI)
		, actualImage(inActualImage)
		, bDiffuseOnly(bInDiffuseOnly)
		, bUseSTBN(bInUseSTBN)
	{
	}
...
protected:
	...
	bool onTick()
	{
		...
		if (bNeedReadback)
		{
			rendererOptions.bufferVisualization = bDiffuseOnly ? EBufferVisualizationMode::IndirectDiffuse : EBufferVisualizationMode::None;
		}
		...
		if (bNeedReadback)
		{
			...
			const char* sDebugVis = bDiffuseOnly ? "indirectDiffuseOnly" : "fullColor";
			const char* sUseSTBN = bUseSTBN ? "stbn" : "rng";

			char msg[256]; std::wstring wMsg;
			sprintf_s(msg, "TestIndirectDiffuse/%s/%s_%s_%d.png", sApi, sDebugVis, sUseSTBN, frameCounter);
			str_to_wstr(msg, wMsg);
			actualTag = wMsg;
			sprintf_s(msg, "TestIndirectDiffuse/%s_%s_%d.png", sDebugVis, sUseSTBN, frameCounter);
			str_to_wstr(msg, wMsg);
			refTag = wMsg;
...
private:
	void createScene()
	{
		...
		for (const auto& baseTex : baseTextures)
		{
			...
			material->setAlbedoMultiplier(vec3(0.2f));
		...
		// Ground
		{
			...
			M_ground->setAlbedoTexture(baseTextures[0]);
			M_ground->setAlbedoMultiplier(vec3(0.5f, 0.2f, 0.4f));
...
private:
	...
	ActualImage_IndirectDiffuse* actualImage = nullptr;
};

namespace UnitTest
{
	template<ERenderDeviceRawAPI graphicsAPI>
	class TestIndirectDiffuseBase
	{
	protected:
		// bDiffuseOnly: Capture only indirect diffuse lighting.
		void RenderIndirectDiffuse(bool bDiffuseOnly, bool bUseSTBN)
		{
			ActualImage_IndirectDiffuse actualImage;

			HWND nativeWindowHandle = NULL;

			CysealEngine cysealEngine;

			ConsoleApplication* app = new IndirectDiffuseApplication(&cysealEngine, graphicsAPI, &actualImage, bDiffuseOnly, bUseSTBN);

			ApplicationCreateParams createParams;
			createParams.nativeWindowHandle = nativeWindowHandle;
			createParams.applicationName = L"TestIndirectDiffuse";
	...
	TEST_CLASS(TestIndirectDiffuseD3D12), TestIndirectDiffuseBase<ERenderDeviceRawAPI::DirectX12>
	{
	public:
		TEST_METHOD(RenderIndirectDiffuseRNG)
		{
			TestIndirectDiffuseBase::RenderIndirectDiffuse(false, false);
		}
		TEST_METHOD(RenderIndirectDiffuseSTBN)
		{
			TestIndirectDiffuseBase::RenderIndirectDiffuse(false, true);
		}
		TEST_METHOD(RenderIndirectDiffuseDebugVisRNG)
		{
			TestIndirectDiffuseBase::RenderIndirectDiffuse(true, false);
		}
		TEST_METHOD(RenderIndirectDiffuseDebugVisSTBN)
		{
			TestIndirectDiffuseBase::RenderIndirectDiffuse(true, true);
		}
	};

	// #todo-test: VK can't run SceneRenderer yet.
#if 0
	TEST_CLASS(TestIndirectDiffuseVulkan), TestIndirectDiffuseBase<ERenderDeviceRawAPI::Vulkan>
	{
	public:
		TEST_METHOD(RenderIndirectDiffuse)
		{
			TestIndirectDiffuseBase::RenderIndirectDiffuse();
		}
	};
#endif
}
