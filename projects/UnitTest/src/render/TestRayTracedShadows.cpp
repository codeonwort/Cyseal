...
class RayTracedShadowsApplication : public ConsoleApplication
{
public:
	RayTracedShadowsApplication(CysealEngine* inEngine, ERenderDeviceRawAPI inGraphicsAPI, ActualImage_RayTracedShadows* inActualImage)
...
protected:
	...
	bool onTick()
	{
		...
		if (bNeedReadback)
		{
			...
			actualTag = wMsg;
			sprintf_s(msg, "TestRayTracedShadows/%s.png", sDebugVis);
			str_to_wstr(msg, wMsg);
			refTag = wMsg;
...
namespace UnitTest
{
	...
	class TestRayTracedShadowsBase
	{
	protected:
		void RenderRayTracedShadows()
		{
			ActualImage_RayTracedShadows actualImage;

			HWND nativeWindowHandle = NULL;

			CysealEngine cysealEngine;

			ConsoleApplication* app = new RayTracedShadowsApplication(&cysealEngine, graphicsAPI, &actualImage);

			ApplicationCreateParams createParams;
			createParams.nativeWindowHandle = nativeWindowHandle;
			createParams.applicationName = L"TestRayTracedShadows";
	...
	TEST_CLASS(TestRayTracedShadowsD3D12), TestRayTracedShadowsBase<ERenderDeviceRawAPI::DirectX12>
	{
	public:
		TEST_METHOD(RenderRayTracedShadows)
		{
			TestRayTracedShadowsBase::RenderRayTracedShadows();
		}
	};

	// #todo-test: VK can't run SceneRenderer yet.
#if 0
	TEST_CLASS(TestRayTracedShadowsVulkan), TestRayTracedShadowsBase<ERenderDeviceRawAPI::Vulkan>
	{
	public:
		TEST_METHOD(RenderRayTracedShadows)
		{
