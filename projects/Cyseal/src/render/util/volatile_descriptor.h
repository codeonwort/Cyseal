...
public:
	...
	void initialize(RenderDevice* inRenderDevice, const wchar_t* inPassName, uint32 swapchainCount, uint32 uniformTotalSize, uint32 uniformChunkCount = 1);
	...
	inline ConstantBufferView* getUniformCBV(uint32 swapchainIndex)
	{
		CHECK(uniformCBVs.bufferCount() > 0); // uniformTotalSize was 0 in initialize().
		CHECK(chunkCount == 1);
		return uniformCBVs.at(swapchainIndex, 0);
	}

	inline ConstantBufferView* getUniformChunkCBV(uint32 swapchainIndex, uint32 chunkIndex)
	{
		CHECK(uniformCBVs.bufferCount() > 0); // uniformTotalSize was 0 in initialize().
		CHECK(chunkCount > 1 && chunkIndex < chunkCount);
		return uniformCBVs.at(swapchainIndex, chunkIndex);
	}
...
private:
	...
	BufferedUniquePtrVec<ConstantBufferView> uniformCBVs; // size = swapchain count
	uint32 chunkCount;
};
