...
public:
	...
	inline bool operator!=(const vec3& v2) const { return !(*this == v2); }
...
{
	return vec3((float)fabs(v.x), (float)fabs(v.y), (float)fabs(v.z));
}
...
inline bool allLessThan(const vec3& v1, const vec3& v2)
{
	return v1.x < v2.x && v1.y < v2.y && v1.z < v2.z;
}
